...
function tokenExpiryStatus(expiry: string | null): 'ok' | 'warn' | 'expired' {
  if (!expiry) return 'ok'; // no expiry = long-lived page token, treat as ok
  const msLeft = new Date(expiry).getTime() - Date.now();
  if (msLeft < 0) return 'expired';
  if (msLeft < 7 * 24 * 60 * 60 * 1000) return 'warn'; // < 7 days
  return 'ok';
}
                  ...
                  <div className="mt-3 space-y-2">
                    {connected.map((acc) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`w-2 h-2 rounded-full shrink-0 ${expiry === 'expired' ? 'bg-red-500' : expiry === 'warn' ? 'bg-amber-400' : 'bg-green-500'}`} />
                          <span className="text-xs font-semibold text-slate-800 flex-1 truncate">{acc.accountName}</span>
                          {expiry === 'expired' && (
                            <span className="text-[9px] font-bold text-red-600 bg-red-100 px-1.5 py-0.5 rounded-full shrink-0">Reconnect</span>
                          )}
                          {expiry === 'warn' && (
                            <span className="text-[9px] font-bold text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded-full shrink-0">Expiring</span>
                          )}
                          <button
                  ...
                  return (
                    <tr key={acc.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-5 py-3.5 font-medium text-slate-700">
                        <div className="flex items-center gap-2">
                          {acc.accountName}
                          {tokenExpiryStatus(acc.tokenExpiry) === 'expired' && (
                            <span className="text-[9px] font-bold text-red-600 bg-red-50 border border-red-200 px-1.5 py-0.5 rounded-full">Reconnect</span>
                          )}
                          {tokenExpiryStatus(acc.tokenExpiry) === 'warn' && (
                            <span className="text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-200 px-1.5 py-0.5 rounded-full">Expiring soon</span>
                          )}
                        </div>
                      </td>
                      <td className="px-5 py-3.5">
